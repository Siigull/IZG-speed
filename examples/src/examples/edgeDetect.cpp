...
void vfx_vs(OutVertex&outVertex,InVertex const&inVertex,ShaderInterface const&){
  outVertex.gl_Position = glm::vec4(0.f,0.f,0.f,1.f);
...
Method::Method(GPUMemory&m,MethodConstructionData const*): ::Method(m){
  colorBuffer = TextureData(m.framebuffers[0].width,m.framebuffers[0].height,3,Image::U8 );
  depthBuffer = TextureData(m.framebuffers[0].width,m.framebuffers[0].height,1,Image::F32);
  ...
  mem.programs[0].vs2fs[3]       = AttribType::VEC4;
  ...
  mem.programs[1].vs2fs[0]       = AttribType::VEC2;
