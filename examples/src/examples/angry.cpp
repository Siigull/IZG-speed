...
Method::Method(GPUMemory&m,MethodConstructionData const*): ::Method(m){
  ...
  prg.vs2fs[0]       = AttribType::VEC2;
  prg.vs2fs[1]       = AttribType::VEC3;
  prg.vs2fs[2]       = AttribType::UINT;

  BlendingSettings settings;
  settings.enabled = true                          ;
  settings.sFactor = BlendFunc::SRC_ALPHA          ;
  settings.dFactor = BlendFunc::ONE_MINUS_SRC_ALPHA;
  pushSetBlendingCommand(commandBuffer,settings);
...
REGISTER_METHOD("students when they see the izg project");
