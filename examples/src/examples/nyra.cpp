...
Method::Method(GPUMemory&m,MethodConstructionData const*): ::Method(m){
  ...
  modelData.load(CMAKE_ROOT_DIR "/resources/models/nyra/scene.gltf");
...
void Method::onDraw(SceneParam const&sceneParam){
  ...
  mem.uniforms[getUniformLocation(0,AMBIENT_LIGHT_COLOR   )].v3 = glm::vec4(0xd4,0x6d,0x63,0xff)/512.f;
  mem.uniforms[getUniformLocation(0,LIGHT_COLOR           )].v3 = glm::vec3(1.f);
  gpuRun(mem,drawCB);
}

REGISTER_METHOD("nyra");
