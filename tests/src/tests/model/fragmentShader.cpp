...
struct FragInput{
  ...
  void computeOutFragments(OutFragment&expF,OutFragment&stuF)const;
...
void FragInput::computeOutFragments(OutFragment&expF,OutFragment&stuF)const{
  InFragment inF;
  ...
  uniforms[getUniformLocation(drawID,LIGHT_POSITION     )].v3 = lPos       ;
  uniforms[getUniformLocation(drawID,CAMERA_POSITION    )].v3 = cam        ;
  uniforms[getUniformLocation(drawID,SHADOWMAP_ID       )].i1 = -1         ;
  uniforms[getUniformLocation(drawID,AMBIENT_LIGHT_COLOR)].v3 = aLC        ;
  uniforms[getUniformLocation(drawID,LIGHT_COLOR        )].v3 = dLC        ;
  uniforms[getUniformLocation(drawID,DIFFUSE_COLOR      )].v4 = diffC      ;
  uniforms[getUniformLocation(drawID,TEXTURE_ID         )].i1 = textureID  ;
  uniforms[getUniformLocation(drawID,DOUBLE_SIDED       )].v1 = doubleSided;

  for(size_t k=0;k<texs.size();++k)
    textures[k] = texs[k];

  switchToTeacherSolution();
  drawModel_fragmentShader(expF,inF,si);
  switchToStudentSolution();
  drawModel_fragmentShader(stuF,inF,si);
}
...
void FragInput::printInputs()const{
  ...
  si.uniforms[getUniformLocation(drawID,LIGHT_POSITION     )].v3 = /* pozice světla                                                               */ )."<< str(lPos       )<<R".(; 
  si.uniforms[getUniformLocation(drawID,CAMERA_POSITION    )].v3 = /* pozice kamery                                                               */ )."<< str(cam        )<<R".(; 
  si.uniforms[getUniformLocation(drawID,SHADOWMAP_ID       )].i1 = /* číslo textury, která obsahuje stínovou mapu, nebo -1 pokud stíny nejsou     */ )."<<    (shadowId   )<<R".(; 
  si.uniforms[getUniformLocation(drawID,AMBIENT_LIGHT_COLOR)].v3 = /* barva ambientního světla                                                    */ )."<< str(aLC        )<<R".(; 
  si.uniforms[getUniformLocation(drawID,LIGHT_COLOR        )].v3 = /* barva světla                                                                */ )."<< str(dLC        )<<R".(; 
  si.uniforms[getUniformLocation(drawID,DIFFUSE_COLOR      )].v4 = /* difuzní barva materialu, - pokud není textura                               */ )."<< str(diffC      )<<R".(; 
  si.uniforms[getUniformLocation(drawID,TEXTURE_ID         )].i1 = /* číslo textury s barvou nebo -1 pokud textura není.                          */ )."<< str(textureID  )<<R".(; 
  si.uniforms[getUniformLocation(drawID,DOUBLE_SIDED       )].v1 = /* 0.f znamená, že je to jednostraný povrch, 1.f znamená, že je to doubleSided */ )."<< str(doubleSided)<<R".(; 
  si.textures[] = {...}; // textury
...
void FragInput::testLambert(std::function<void()>const&msg)const{
  OutFragment expF;
  OutFragment stuF;
  computeOutFragments(expF,stuF);



  if(breakTest() || expF.discard != stuF.discard){
    printDiscardInfo(expF,stuF);
    REQUIRE(false);
  }

  //do not check colors when fragment is being discarded
  if(expF.discard)return;

  if(breakTest() || !equalVec4(expF.gl_FragColor,stuF.gl_FragColor)){
    printColorInfo(expF,stuF,msg);
    REQUIRE(false);
  }
}
...
SCENARIO(TEST_NAME){
  ...
  auto sm = TextureData(10,10,1,Image::F32);
  clearTex(sm,10.f);
