...
struct TestModel: public Model{
  TestModel();
  TestModel(TestModel const&m);
  TestModel&operator=(TestModel const&m);
  ~TestModel();
  void copy(TestModel const&m);
};
...
TestModel createModel(std::vector<MeshI>const&meshes,std::vector<NodeI>const&roots);
