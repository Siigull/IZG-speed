...
AllocatedMem::AllocatedMem(AllocatedMem const&m){
  backings.clear();
  shallowCopy(mem,m.mem);
  allocate();
  copyData(mem,m.mem);
}

AllocatedMem&AllocatedMem::operator=(AllocatedMem const&m){
  backings.clear();
  shallowCopy(mem,m.mem);
  allocate();
  copyData(mem,m.mem);
...
void AllocatedMem::putObjectBackingIntoMem(uint8_t*backing,MemBacking::Object const&o){
  ...
  switch(o.type){
    ...
    case MemBacking::Object::FRAMEBUFFER_STENCIL:mem.framebuffers[o.id].stencil.data = ptr;break;
  }
}
...
void AllocatedMem::insertFramebuffersPointers(){
  for(uint32_t i=0;i<mem.maxFramebuffers;++i){
    ...
    insertImagePointer(f.stencil,f.height,MemBacking::Object::FRAMEBUFFER_STENCIL,i);
  }
}
...
void shallowCopy(GPUMemory&dst,GPUMemory const&src){
  ...
  dst.gl_DrawID            = src.gl_DrawID           ;
  dst.stencilSettings      = src.stencilSettings     ;
  dst.blockWrites          = src.blockWrites         ;
...
void copyData(Framebuffer&d,Framebuffer const&s){
  ...
  copyData(d.stencil,s.stencil,s.height);
}
...
void shallowCopy(Framebuffer&d,Framebuffer const&s){
  ...
  shallowCopy(d.stencil,s.stencil);
}
