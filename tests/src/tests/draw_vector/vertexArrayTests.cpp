...
void attributeTest(uint32_t offset = 0,uint32_t stride = sizeof(float)){
  ...
  prg.fragmentShader = fragmentEmpty;

  auto&vao = mem.vertexArrays[4];
  vao.vertexAttrib[0].bufferID   = 0;
  vao.vertexAttrib[0].type       = AttribType::FLOAT;
  ...
  pushDrawCommand(cb,(uint32_t)nofVerts);
...
SCENARIO(TEST_NAME){
  printTestName("vertex shader, attributes");
...
SCENARIO(TEST_NAME){
  printTestName("vertex shader, attributes, offset, stride, multiple buffers");
...
SCENARIO(TEST_NAME){
  printTestName("vertex shader, attributes, offset, stride, indexing");
