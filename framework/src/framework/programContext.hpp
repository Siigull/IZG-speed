...
class MethodDatabase{
  private:
    ...
    struct MethodMetadata{
      ...
      std::string                             name            ;
      std::shared_ptr<MethodConstructionData> constructionData;
    };
    ...
    template<typename CLASS>
    friend void registerMethod(std::string const&name,std::shared_ptr<MethodConstructionData>const&mcd);
    friend class Application;
    friend void renderMethodFrame(Framebuffer&frame);
    friend void getNofExamples();
};
...
void registerMethod(std::string const&name,std::shared_ptr<MethodConstructionData>const&mcd){
  ...
  mr.methodNames        .push_back(finalName);
...
#define REGISTER_METHOD(name)\
  EntryPoint main = [](){registerMethod<Method>(name);}
